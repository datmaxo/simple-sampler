...
import sampler.main_ui as m
import sampler.settings as settings
import sampler.keybind as keybind
...
class startUI:
    def __init__ (self, path):
        ...
        try:
            with open(os.path.join(self.path, 'data/settings.json'), 'r+') as argfile:
                kb = argfile.read()
                if len(kb) == 0:
            ...
            with open(os.path.join(self.path,'data/settings.json'), 'w') as argfile:
                print("No settings file found - saving defaults.")
                argfile.write(json.dumps(self.rec_args))
    ...
    def close (self, _=''):
        ...
        m.MainEditor(self.path, self.rec_args, self.binds, root=self.root)

    def loadIntroFrame (self):
        with open(os.path.join(self.path, 'data/intro-text.txt'), "r+") as info:
            text = info.read()

            introLabel = tk.Label(self.infoframe, text="\nWelcome to Pillow's Simple Sample Recorder! (v1.0)\n",
    ...
    def loadSettingsFrame (self):
        settings.SettingsWin(self.path, self.rec_args, self.recordingframe, self.updateSettings)

    def loadKeybindFrame (self):
        keybind.KeybindsWin(self.path, self.keybindframe, self.loadBinds)
